...
describe('SummaryService', () => {
  ...
  it('should test data step', () => {
    ...
    expect(summary.status.includes('dmp.steps.summary.data.specify.datasets.produced')).toBe(true);
    expect(summary.status.includes('dmp.steps.summary.data.specify.datasets.datageneration')).toBe(
      true,
    );
    expect(summary.status.includes('1. ')).toBe(true);
    ...
    expect(summary.completeness).toEqual(50);
    expect(
      summary.status.includes('dmp.steps.summary.data.specify.datasets.unknown.produced'),
    ).toBe(true);
  });
